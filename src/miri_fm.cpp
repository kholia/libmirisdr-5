...
PskReporter reporter("VU3CER", "MK68xm", "BharatSDR 0.1");
...
void call_python_parser(const char *message) {
    char command[MAX_MESSAGE_LENGTH + 128]; // Buffer for the command
    FILE *fp;

    // Construct the command to execute the Python script
    snprintf(command, sizeof(command), "python3 wspr_parser.py \"%d\" \"%s\"", old_frequency, message);
...
void parse_wspr_line(const char *line) {
	call_python_parser(line);
}
...
static void *controller_thread_fn(void *arg)
{
	...
	const char* file_path = expanduser("~/.local/share/WSPR/ALL_WSPR.TXT");
